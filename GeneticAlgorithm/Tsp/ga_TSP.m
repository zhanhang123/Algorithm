...
CityNum=31; %you chan choose 10, 30, 50, 75
...
%         end
%     end
    city30=[1304 2312; 3639 1315; 4177 2244; 3712 1399;
    ...
    2545 2357; 2778 2826; 2370 2975];%30 cities d'=423.741 by D B Fogel
    for i=1:31
        for j=1:31
            DLn(i,j)=((city30(i,1)-city30(j,1))^2+(city30(i,2)-city30(j,2))^2)^0.5;
        end
...
if f==0&&CityNum~=10
    text(450,200,['第 ',int2str(p),' 代','  最短距离为 ',num2str(bsf)]);
%     text(5,5,['第 ',int2str(p),' 代','  最短距离为 ',num2str(bsf)]);
else
    text(450,200,['最终搜索结果：最短距离 ',num2str(bsf),'， 在第 ',num2str(p),' 代达到']);
end
if CityNum==10
    if f==0
